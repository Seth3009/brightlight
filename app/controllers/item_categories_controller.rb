class ItemCategoriesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @item_category.save
        format.html { redirect_to item_categories_url, notice: 'Item category was successfully created.' }
  ...
  def update
    respond_to do |format|
      if @item_category.update(item_category_params)
        format.html { redirect_to item_categories_url, notice: 'Item category was successfully updated.' }
  ...
  def destroy
    if @item_category.destroy
      ...
      respond_to do |format|
        format.html { redirect_to item_categories_url, alert: @item_category.errors.full_messages.join(" ") }
        format.json { head :no_content }
      end
    end
    # @product = Product.where('item_unit_id = ?',@item_category.id).first
    # if @product.nil?
    ...
    # else
    #   respond_to do |format|
    #     format.html { redirect_to item_categories_url, notice: 'Cannot delete this category, used by some products' }
    #   end
    # end
  end
  ...
  private
    ...
    def item_category_params
      params.require(:item_category).permit(:code, :name)
    end
end
