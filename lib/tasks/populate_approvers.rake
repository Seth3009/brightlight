namespace :data do
  ...
  task populate_approvers: :environment do
    ...
    Department.where(id: [5,6,7,11,12]).each {|d| Approver.create employee_id:431, department_id:d.id, category: 'LR', level: 2 }
    Department.where(id: [1,2,3,5,8,9,10]).each {|d| Approver.create employee_id:46, department_id:d.id, category: 'LR', level: 2 }
