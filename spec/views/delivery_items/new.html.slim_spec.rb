...
RSpec.describe "delivery_items/new", type: :view do
  before(:each) do
    assign(:delivery_item, DeliveryItem.new(
      :delivery => "",
      :order_item => "",
      :quantity => "",
      :unit, => "MyString",
  ...
  it "renders new delivery_item form" do
    render

    assert_select "form[action=?][method=?]", delivery_items_path, "post" do
